...
namespace FoT3\FluidLaravel\ViewHelpers;
     ...
     *                                                                        */
use FoT3\FluidLaravel\ViewHelpers\Form\AbstractFormFieldViewHelper;
 ...
 * @api
 */
class FormViewHelper extends AbstractFormFieldViewHelper
{
     ...
     * @param mixed   $object                               object to use for the form. Use in conjunction with the
     *                                                      "property" attribute on the sub tags
     * @param string  $section                              The anchor to be added to the action URI (only active if
     *                                                      $actionUri is not set)
     * @param string  $format                               The requested format (e.g. ".html") of the target page
     *                                                      (only active if $actionUri is not set)
     * @param array   $additionalParams                     additional action URI query parameters that won't be
     *                                                      prefixed like $arguments (overrule $arguments) (only active
     *                                                      if $actionUri is not set)
     * @param boolean $absolute                             If set, an absolute action URI is rendered (only active if
     *                                                      $actionUri is not set)
     * @param boolean $addQueryString                       If set, the current query parameters will be kept in the
     *                                                      action URI (only active if $actionUri is not set)
     * @param array   $argumentsToBeExcludedFromQueryString arguments to be removed from the action URI. Only active if
     *                                                      $addQueryString = TRUE and $actionUri is not set
     * @param string  $fieldNamePrefix                      Prefix that will be added to all field names within this
     *                                                      form
     * @param string  $actionUri                            can be used to overwrite the "action" attribute of the form
     *                                                      tag
     * @param string  $objectName                           name of the object that is bound to this form. If this
     *                                                      argument is not specified, the name attribute of this form
     *                                                      is used to determine the FormObjectName
     * @param boolean $useParentRequest                     If set, the parent Request will be used instead ob the
     *                                                      current one
     *
...
//            throw new \Exception('FormViewHelper requires "actionUri" or "action" argument to be specified',
//                1355243748);
//        }
...
//        $content .= $this->renderHiddenIdentityField($this->arguments['object'], $this->getFormObjectName());
        ...
        if ($this->hasArgument('actionUri')) {
            ...
            if ($this->arguments['useParentRequest'] === true) {
                ...
                if ($request->isMainRequest()) {
                    throw new \Exception('You can\'t use the parent Request, you are already in the MainRequest.',
                        1361354942);
                }
            ...
            try {
                $this->formActionUri = $uriBuilder
                    ->uriFor($this->arguments['action'], $this->arguments['arguments'], $this->arguments['controller'],
                        $this->arguments['package'], $this->arguments['subpackage']);
        ...
        if (is_string($query)) {
            ...
            foreach ($queryParts as $queryPart) {
                if (strpos($queryPart, '=') !== false) {
                    ...
                    $result .= chr(10) . '<input type="hidden" name="' . htmlspecialchars(urldecode($parameterName)) .
                        '" value="' . htmlspecialchars(urldecode($parameterValue)) . '" />';
                }
            }
        }
     ...
     * This happens if a form field is defined like property="bla.blubb" - then we might need an identity property for
     * the sub-object "bla".
     *
    ...
    {
        if ($this->viewHelperVariableContainer->exists('TYPO3\Fluid\ViewHelpers\FormViewHelper',
            'additionalIdentityProperties')
        ) {
            $additionalIdentityProperties =
                $this->viewHelperVariableContainer->get('TYPO3\Fluid\ViewHelpers\FormViewHelper',
                    'additionalIdentityProperties');
            $output = '';
...
//                $result .= '<input type="hidden" name="' . $argumentNamespace . '[__referrer][' . $referrerKey .
//                    ']" value="' . $referrerValue . '" />' . chr(10);
//            }
...
//
//        foreach ($referrer as $referrerKey => $referrerValue) {
//            $result .= '<input type="hidden" name="__referrer[' . $referrerKey . ']" value="' .
//                htmlspecialchars($referrerValue) . '" />' . chr(10);
//        }
    ...
    /**
     * Adds the form object name to the ViewHelperVariableContainer if "objectName" argument or "name" attribute is
     * specified.
     *
     * @return void
     */
    protected function addFormObjectNameToViewHelperVariableContainer()
    {
        ...
        if ($formObjectName !== null) {
            $this->viewHelperVariableContainer->add('TYPO3\Fluid\ViewHelpers\FormViewHelper', 'formObjectName',
                $formObjectName);
        }
    }
    ...
    /**
     * Adds the object that is bound to this form to the ViewHelperVariableContainer if the formObject attribute is
     * specified.
     *
     * @return void
     */
    protected function addFormObjectToViewHelperVariableContainer()
    {
        if ($this->hasArgument('object')) {
            $this->viewHelperVariableContainer->add('TYPO3\Fluid\ViewHelpers\FormViewHelper', 'formObject',
    ...
    protected function removeFormObjectFromViewHelperVariableContainer()
    {
        if ($this->hasArgument('object')) {
            ...
            $this->viewHelperVariableContainer->remove('TYPO3\Fluid\ViewHelpers\FormViewHelper',
                'additionalIdentityProperties');
        }
    }
    ...
    protected function addFieldNamePrefixToViewHelperVariableContainer()
    {
        ...
        $this->viewHelperVariableContainer->add('TYPO3\Fluid\ViewHelpers\FormViewHelper', 'fieldNamePrefix',
            $fieldNamePrefix);
    }
...
//        }
    ...
    {
        $this->viewHelperVariableContainer->add('TYPO3\Fluid\ViewHelpers\FormViewHelper', 'emptyHiddenFieldNames',
            array());
    }
    ...
    protected function renderEmptyHiddenFields()
    {
        ...
        if ($this->viewHelperVariableContainer->exists('TYPO3\Fluid\ViewHelpers\FormViewHelper',
            ...
            $emptyHiddenFieldNames = $this->viewHelperVariableContainer->get('TYPO3\Fluid\ViewHelpers\FormViewHelper',
                'emptyHiddenFieldNames');
            foreach ($emptyHiddenFieldNames as $hiddenFieldName) {
                $result .= '<input type="hidden" name="' . htmlspecialchars($hiddenFieldName) . '" value="" />' .
                    chr(10);
            }
        }
    ...
    {
//        $formFieldNames =
//            $this->viewHelperVariableContainer->get('TYPO3\Fluid\ViewHelpers\FormViewHelper', 'formFieldNames');
//        $requestHash = $this->mvcPropertyMappingConfigurationService->generateTrustedPropertiesToken($formFieldNames,
//            $this->getFieldNamePrefix());
//
//        return '<input type="hidden" name="' . $this->prefixFieldName('__trustedProperties') . '" value="' .
//        htmlspecialchars($requestHash) . '" />' . chr(10);
        return '';
    }
