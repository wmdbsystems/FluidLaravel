...
abstract class AbstractFormViewHelper extends AbstractTagBasedViewHelper
{
    ...
    protected function prefixFieldName($fieldName)
    {
        ...
        $fieldNamePrefix = (string)$this->viewHelperVariableContainer->get('TYPO3\Fluid\ViewHelpers\FormViewHelper',
            'fieldNamePrefix');
        if ($fieldNamePrefix === '') {
            return $fieldName;
        }
     ...
     *
     * @return string A hidden field containing the Identity (UUID in TYPO3 Flow, uid in Extbase) of the given object
     *                or NULL if the object is unknown to the persistence framework
    ...
    protected function registerFieldNameForFormTokenGeneration($fieldName)
    {
        ...
        $this->viewHelperVariableContainer->addOrUpdate('TYPO3\Fluid\ViewHelpers\FormViewHelper', 'formFieldNames',
            $formFieldNames);
    }
}
